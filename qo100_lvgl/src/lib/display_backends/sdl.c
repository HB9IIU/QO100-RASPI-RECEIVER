...
static lv_display_t * init_sdl(void)
{
    ...
    /*
     * The generic simulator stores -f/-m in `settings`, but the stock SDL
     * backend does not apply those settings to its window. Do it here after
     * LVGL has created the SDL window.
     *
     * FULLSCREEN_DESKTOP keeps the Pi desktop's current video mode while
     * removing the title bar and borders. This works with Wayland and X11.
     */
    SDL_Window * window = lv_sdl_window_get_window(disp);
    if(settings.fullscreen) {
        SDL_SetWindowBordered(window, SDL_FALSE);
        if(SDL_SetWindowFullscreen(window, SDL_WINDOW_FULLSCREEN_DESKTOP) != 0) {
            LV_LOG_WARN("SDL fullscreen failed: %s", SDL_GetError());
        }
    }
    ...
    /*
     * This application is operated by touchscreen, so do not attach the
     * simulator's mouse cursor image. Pointer/touch events still work.
     */
