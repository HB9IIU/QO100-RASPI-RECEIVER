...
static void web_status_json(char **status_string_ptr, longmynd_status_t *status, longmynd_status_t *status_cache)
{
    ...
    /* json_object_to_json_string() returns storage owned by statusObj.
     * Copy it before json_object_put() destroys that storage; returning the
     * borrowed pointer made the monitor WebSocket send freed/corrupt data. */
